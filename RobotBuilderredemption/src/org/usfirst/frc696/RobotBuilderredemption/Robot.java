...
public class Robot extends IterativeRobot {
    ...
    public void teleopPeriodic() {
        //ln("cheesy:   "+Robot.driveTrain.server.getLeftStatus()+ "  " + Robot.driveTrain.server.getRightStatus());
        ...
        //drive code
           ...
           // if (button[6] && (!oldButton[6])) {
            ...
            lastLeftLimitSwitch = RobotMap.shooterchoochooLimitSwitchLeft.get();
            lastRightLimitSwitch = RobotMap.shooterchoochooLimitSwitchRight.get();
            
        } else {
            System.out.println(RobotMap.driveTraincompressor.enabled());
            ...
            RobotMap.lEDsblue.updateDutyCycle(pinkLight / 2);
            double period = 2;
            double cycleTime = teleopTimer.get()%period;
            if(cycleTime >0.1*period&&cycleTime<0.4*period){
                Robot.lEDs.setleds(1, 0, 0);
            }else if(cycleTime <0.1*period){
                Robot.lEDs.setleds(cycleTime*5, 0, 0);
            }else if(cycleTime >0.4*period && cycleTime<0.5*period){
                Robot.lEDs.setleds(1-((cycleTime-0.8)*5), 0, 0);
